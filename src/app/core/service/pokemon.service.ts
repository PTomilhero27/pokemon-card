...
import { SetPokemon, SetsData } from 'src/app/models/sets';
...
export class PokemonService {
  ...
  getCards(page: number = 1, pageSize: number = 10): Observable<Card[]> {
    return this.http
      .get<CardData>(`${this.apiUrl}/cards`, {
  ...
  getSets(page: number = 1, pageSize: number = 10): Observable<SetPokemon[]> {
    return this.http
      .get<{ data: SetPokemon[] }>(`${this.apiUrl}/sets`, {
        params: { page: page.toString(), pageSize: pageSize.toString() },
      })
      .pipe(
        map((response) => response.data),
